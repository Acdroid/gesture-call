...
public class Rater {
	...
	public static void checkRater (Context context){
		...
		try {
			...
			if (counter >= APP_USES * ap.getInt(AppConfig.RATER_REMIND_LATER)){
			...
			if (System.currentTimeMillis() >= dateFirstLauch + ( APP_DAYS_USES * ap.getInt(AppConfig.RATER_REMIND_LATER) * 24 * 60 * 60 * 1000)){
				showRaterDialog(context);
				return;
			}		
			...
			ap.put(1, AppConfig.RATER_REMIND_LATER);
	...
	public static void checkRater (Context context ,int appUses,int daysUses){
		...
		try {
			...
			int counter = ap.getInt(AppConfig.RATER_LAUCH_COUNTER) + 1;
			Log.d("DEBUG","Counter " + counter + "  remind " + ap.getInt(AppConfig.RATER_REMIND_LATER));
			ap.put(counter, AppConfig.RATER_LAUCH_COUNTER );
			if (counter >= appUses * ap.getInt(AppConfig.RATER_REMIND_LATER)){
				showRaterDialog(context);
				return;
			}

			Long dateFirstLauch = ap.getLong(AppConfig.RATER_FIRST_USE);
			if (System.currentTimeMillis() >= dateFirstLauch + ( daysUses * ap.getInt(AppConfig.RATER_REMIND_LATER) * 24 * 60 * 60 * 1000)){
				showRaterDialog(context);
				return;
			}		
